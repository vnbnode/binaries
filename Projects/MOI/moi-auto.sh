...
if [ "$version" != "2" ];then
	...
	sudo docker run --name $container_name_moi -p 6000:6000/tcp -p 6000:6000/udp -p 1600:1600 -it -d -w /data -v $(pwd):/data sarvalabs/moipod:v0.3.0-port server --babylon --data-dir $moi_dirpath --log-level DEBUG --node-password $moi_passwd
fi
sleep 1
