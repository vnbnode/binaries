...
if [ "$version" != "2" ];then
	...
	sudo docker run --name $container_name_moi --network host -it -d -w /data -v $(pwd):/data sarvalabs/moipod:v0.4.0-port server --babylon --data-dir $moi_dirpath --log-level DEBUG --node-password $moi_passwd
        docker update --restart=unless-stopped $container_name_moi
fi
sleep 1
