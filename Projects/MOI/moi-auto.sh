...
if [ "$version" != "2" ];then
    ...
    rm -r $HOME/moi/db
    rm -r $HOME/moi/consensus
    sudo docker run --name $container_name_moi --network host -it -d -w /data -v $(pwd):/data sarvalabs/moipod:v0.6.4-port server --babylon --data-dir $moi_dirpath --log-level DEBUG --node-password $moi_passwd
    docker update --restart=unless-stopped $container_name_moi
fi
